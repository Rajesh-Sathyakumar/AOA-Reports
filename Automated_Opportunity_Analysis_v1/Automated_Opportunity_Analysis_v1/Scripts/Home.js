                            ...
                            if (Startdate <= Enddate) {
                                ...
                                if (Startdate <= MaxDischargeDate) {
                                    ...
                                    if (Enddate > MaxDischargeDate) {
                                        mesg = $.growl.warning({ message: "Please Note that the Generated Report will contain results only up till the 'Data Loaded Till Date' !" });
                                    }
                                    ...
                                    mesg = $.growl.error({ message: "Error: Please Choose an StartDate that is lesser than the 'Data Loaded Till' Date !" });
                                }
                                
                            }
                            else {
                                mesg = $.growl.error({ message: "Error: Please Choose an EndDate that is greater than Startdate!" });
                            }
                        }
                        else {
                            mesg = $.growl.error({ message: "Error: Please select a valid End Date!" });
                        }
                    }
                    else {
                        mesg = $.growl.error({ message: "Error: Please select a valid Startdate!" });
                    }
                }
                else {
                    mesg = $.growl.error({ message: "Error: Please select a valid Facility!" });
                }
            }

            else {
                mesg = $.growl.error({ message: "Error: You have not selected a Member. Please select a Member Name for Report Generation !" });
            }
